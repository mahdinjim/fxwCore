...
class DesignerController extends Controller
{
    public function CreateAction()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $creds->setTitle(Titles::Designer);
                ...
                if (count($errorList) > 0 || count($crederrorlist)>0) {
                    ...
                    $res->setContent(ConstValues::DESCREATED);
                    return $res;
                }
    ...
    public function UpdateAction()
    {
        ...
        else
        {
            ...
            {
                if ($this->get('security.context')->isGranted('ROLE_Designer') && $this->get('security.context')->getToken()->getUser()->getId()!=$json->{'id'})  {
                        $response=new Response();
                ...
                {
                    $user=$em->getRepository("AcmtoolAppBundle:Designer")->findOneById($json->{'id'});
                    if($user instanceOf Designer){
                        ...
                        if (count($errorList) > 0 || count($crederrorlist)>0) {
                            ...
                            $res=new Response();
                            $res->setStatusCode(200);
                            $res->setContent(ConstValues::DESUPDATED);
                            return $res;
                        }
                    }
                    else
    ...
    public function DeleteAction($id)
    {
        ...
        if($user){
            ...
            $res=new Response();
            $res->setStatusCode(200);
            $res->setContent(ConstValues::DESDELETED);
            return $res;
        }
        else
        {
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $totalpages=ceil($em->createQuery("SELECT COUNT(t) FROM AcmtoolAppBundle:Designer t")
        ->getSingleScalarResult()/10);
        $start=ConstValues::COUNT*($page-1);
        $End=ConstValues::COUNT*$page;
        $result=$em->createQuery('select d from AcmtoolAppBundle:Designer d')
