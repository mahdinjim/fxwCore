...
class KeyAccountController extends Controller
{
    ...
    public function CreateAction()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $creds->setTitle(Titles::KeyAccount);
    ...
    public function UpdateAction()
    {
        ...
        else
        {
            ...
            {
                if ($this->get('security.context')->isGranted('ROLE_KEYACCOUNT') && $this->get('security.context')->getToken()->getUser()->getId()!=$json->{'id'})  {
                        $response=new Response();
                ...
                {
                    $user=$em->getRepository("AcmtoolAppBundle:KeyAccount")->findOneById($json->{'id'});
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $totalpages=ceil($em->createQuery("SELECT COUNT(t) FROM AcmtoolAppBundle:KeyAccount t")
        ...
        if(count($result)>0)
        {
            ...
            foreach ($result as $user) {
                $users[$i] = array('id'=>$user->getId(),'username' =>$user->getUsername(),'email'=>$user->getEmail(),'photo'=>$user->getPhoto(),"name"=>$user->getName(),"surname"=>$user->getSurname(),"photo"=>$user->getPhoto(),"description"=>$user->getDescription(),"language"=>$user->getLanguage(),"hourate"=>$user->getHourrate(),"level"=>$user->getLevel());
                $i++;
    ...
    {
        $em = $this->getDoctrine()->getManager();
       if ($this->get('security.context')->isGranted('ROLE_KEYACCOUNT') && $this->get('security.context')->getToken()->getUser()->getId()!=$id)  {
            $response=new Response();
            $response->setStatusCode(403);
            $response->headers->set('Content-Type', 'application/json');
            return $response;
        ...
        else
        {
            ...
            if($user)
            {
                $UserInfo = array('id'=>$user->getId(),'username' =>$user->getUsername(),'email'=>$user->getEmail(),'photo'=>$user->getPhoto(),"name"=>$user->getName(),"surname"=>$user->getSurname(),"photo"=>$user->getPhoto(),"description"=>$user->getDescription());
                $res=new Response();
                $res->setStatusCode(200);
